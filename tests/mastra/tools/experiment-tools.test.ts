...
describe("Experiment Tools", () => {
  ...
  it("creates an experiment in draft status", async () => {
    const expId = await createExperimentFn({
      ...
      ownerAgent: "growth-lead", primaryMetric: "open_rate", minSampleSize: 1000, vertical: "med-spa",
    });
  ...
  it("adds variants to an experiment", async () => {
    const expId = await createExperimentFn({
      ...
      ownerAgent: "growth-lead", primaryMetric: "conversion_rate", vertical: "med-spa",
    });
  ...
  it("starts an experiment (draft → running)", async () => {
    const expId = await createExperimentFn({
      ...
      ownerAgent: "growth-lead", primaryMetric: "reply_rate", vertical: "med-spa",
    });
  ...
  it("evaluates — insufficient data when no results", async () => {
    const expId = await createExperimentFn({
      ...
      primaryMetric: "conversion_rate", minSampleSize: 1000, vertical: "med-spa",
    });
  ...
  it("evaluates — finds winner when thresholds met", async () => {
    const expId = await createExperimentFn({
      ...
      primaryMetric: "conversion_rate", minSampleSize: 100, vertical: "med-spa",
    });
